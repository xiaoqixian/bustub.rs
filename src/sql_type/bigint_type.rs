...
// bigint_type.rs
//
// Identification: src/sql_type/bigint_type.rs
//
...
/// The SQL BIGINT type (8-byte signed integer).
...
impl BigintType {
    pub fn new() -> Self {
        BigintType {
            inner: NumericTypeImpl::new(TypeId::BigInt),
        }
    }

    fn compare_op(left: &Value, right: &Value, op: impl FnOnce(i64, i64) -> bool) -> CmpBool {
        match right.get_type_id() {
            TypeId::TinyInt => get_cmp_bool(op(left.get_as::<i64>(), right.get_as::<i8>() as i64)),
            TypeId::SmallInt => get_cmp_bool(op(left.get_as::<i64>(), right.get_as::<i16>() as i64)),
            TypeId::Integer => get_cmp_bool(op(left.get_as::<i64>(), right.get_as::<i32>() as i64)),
            TypeId::BigInt => get_cmp_bool(op(left.get_as::<i64>(), right.get_as::<i64>())),
            TypeId::Decimal => get_cmp_bool(op(left.get_as::<i64>(), right.get_as::<f64>() as i64)),
            TypeId::Varchar => {
                let r_value = right.cast_as(TypeId::BigInt);
                get_cmp_bool(op(left.get_as::<i64>(), r_value.get_as::<i64>()))
            }
            _ => CmpBool::CmpNull,
        }
    }
    ...
    fn modify_op(left: &Value, right: &Value, op: impl FnOnce(i64, i64) -> i64) -> Value {
        match right.get_type_id() {
            TypeId::TinyInt => {
                Value::from_i64(TypeId::BigInt, op(left.get_as::<i64>(), right.get_as::<i8>() as i64))
            }
            TypeId::SmallInt => {
                Value::from_i64(TypeId::BigInt, op(left.get_as::<i64>(), right.get_as::<i16>() as i64))
            }
            TypeId::Integer => {
                Value::from_i64(TypeId::BigInt, op(left.get_as::<i64>(), right.get_as::<i32>() as i64))
            }
            TypeId::BigInt => {
                Value::from_i64(TypeId::BigInt, op(left.get_as::<i64>(), right.get_as::<i64>()))
            }
            TypeId::Decimal => {
                let l = left.get_as::<i64>() as f64;
                let r = right.get_as::<f64>();
                Value::from_f64(TypeId::Decimal, op(l as i64, r as i64) as f64)
            }
            TypeId::Varchar => {
                let r_value = right.cast_as(TypeId::BigInt);
                Value::from_i64(TypeId::BigInt, op(left.get_as::<i64>(), r_value.get_as::<i64>()))
            }
            _ => panic!("type error"),
        }
    }
...
impl SqlType for BigintType {
    ...
    fn get_min_value(&self) -> Value { Value::from_i64(TypeId::BigInt, BUSTUB_INT64_MIN) }
    fn get_max_value(&self) -> Value { Value::from_i64(TypeId::BigInt, BUSTUB_INT64_MAX) }
    ...
    fn modulo(&self, left: &Value, right: &Value) -> Value {
        ...
        if right.is_zero() { panic!("Division by zero on right-hand side"); }
        match right.get_type_id() {
            TypeId::TinyInt => Value::from_i64(TypeId::BigInt, left.get_as::<i64>() % right.get_as::<i8>() as i64),
            TypeId::SmallInt => Value::from_i64(TypeId::BigInt, left.get_as::<i64>() % right.get_as::<i16>() as i64),
            TypeId::Integer => Value::from_i64(TypeId::BigInt, left.get_as::<i64>() % right.get_as::<i32>() as i64),
            TypeId::BigInt => Value::from_i64(TypeId::BigInt, left.get_as::<i64>() % right.get_as::<i64>()),
            TypeId::Decimal => {
                let l = left.get_as::<i64>() as f64;
                let r = right.get_as::<f64>();
            ...
            TypeId::Varchar => {
                let r_value = right.cast_as(TypeId::BigInt);
                Value::from_i64(TypeId::BigInt, left.get_as::<i64>() % r_value.get_as::<i64>())
            }
            _ => panic!("type error"),
        }
    }
    ...
    fn operate_null(&self, _left: &Value, right: &Value) -> Value {
        match right.get_type_id() {
            TypeId::TinyInt | TypeId::SmallInt | TypeId::Integer | TypeId::BigInt => {
                Value::from_i64(TypeId::BigInt, BUSTUB_INT64_NULL)
            }
    ...
    fn is_zero(&self, val: &Value) -> bool { val.get_as::<i64>() == 0 }
    fn is_inlined(&self, _val: &Value) -> bool { true }

    fn to_string_val(&self, val: &Value) -> String {
        assert!(val.check_integer());
        if val.is_null() { return "bigint_null".to_string(); }
        val.get_as::<i64>().to_string()
    }

    fn serialize_to(&self, val: &Value, storage: &mut [u8]) {
        let v: i64 = val.get_as();
    ...
    fn deserialize_from(&self, storage: &[u8]) -> Value {
        ...
        Value::from_i64(TypeId::BigInt, val)
    }

    fn copy(&self, val: &Value) -> Value {
        Value::from_i64(TypeId::BigInt, val.get_as::<i64>())
    }
    ...
    fn cast_as(&self, val: &Value, type_id: TypeId) -> Value {
        match type_id {
            TypeId::TinyInt => {
                ...
                let v = val.get_as::<i64>();
                if v > BUSTUB_INT8_MAX as i64 || v < BUSTUB_INT8_MIN as i64 { panic!("Numeric value out of range."); }
                Value::from_i8(TypeId::TinyInt, v as i8)
            }
            TypeId::SmallInt => {
                if val.is_null() { return Value::from_i16(TypeId::SmallInt, BUSTUB_INT16_NULL); }
                let v = val.get_as::<i64>();
                if v > BUSTUB_INT16_MAX as i64 || v < BUSTUB_INT16_MIN as i64 { panic!("Numeric value out of range."); }
                Value::from_i16(TypeId::SmallInt, v as i16)
            }
            TypeId::Integer => {
                if val.is_null() { return Value::from_i32(TypeId::Integer, BUSTUB_INT32_NULL); }
                let v = val.get_as::<i64>();
                if v > BUSTUB_INT32_MAX as i64 || v < BUSTUB_INT32_MIN as i64 { panic!("Numeric value out of range."); }
            ...
            TypeId::Decimal => {
                if val.is_null() { return Value::from_f64(TypeId::Decimal, BUSTUB_DECIMAL_NULL); }
                Value::from_f64(TypeId::Decimal, val.get_as::<i64>() as f64)
            }
            TypeId::Varchar => {
                if val.is_null() { return Value::from_bytes(TypeId::Varchar, &[], 0, false); }
                Value::from_string(TypeId::Varchar, &val.to_string_val())
            }
            _ => panic!("bigint is not coercable to {}", type_id_to_string(type_id)),
        }
    }
    ...
    fn get_data<'a>(&self, _val: &'a Value) -> &'a [u8] { panic!("GetData not implemented for BigintType") }
    fn get_storage_size(&self, _val: &Value) -> u32 { panic!("GetStorageSize not implemented for BigintType") }
}
