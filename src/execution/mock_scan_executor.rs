...
use crate::sql_type::{TypeId, Value, ValueFactory};
    ...
    match table {
        "__mock_table_1" => {
            Box::new(move |cursor: usize| {
                let values = vec![
                    Value::from_i32(TypeId::Integer, cursor as i32),
                    Value::from_i32(TypeId::Integer, (cursor * 100) as i32),
                ];
        ...
        "__mock_table_3" => {
            Box::new(move |cursor: usize| {
                let values = if cursor % 2 == 0 {
                    ...
                    vec![
                        ValueFactory::get_null_value_by_type(TypeId::Integer),
                        Value::from_str(format!("{}-💩", cursor).as_str()),
                    ]
                };
                Tuple::new_with_values(values, plan.output_schema_ref())
        ...
        "__mock_table_schedule_2022" => {
            Box::new(move |cursor: usize| {
                let values = vec![
                    ...
                    Value::from_i32(TypeId::Integer, if cursor == 1 || cursor == 3 { 1 } else { 0 }),
                ];
                Tuple::new_with_values(values, plan.output_schema_ref())
            })
        }
        "__mock_table_schedule" => {
            Box::new(move |cursor: usize| {
                let values = vec![
                    Value::from_str(COURSE_ON_DATE[cursor]),
                    Value::from_i32(TypeId::Integer, if cursor == 0 || cursor == 2 { 1 } else { 0 }),
                ];
                Tuple::new_with_values(values, plan.output_schema_ref())
            })
        }
        "__mock_agg_input_small" => {
            Box::new(move |cursor: usize| {
                let values = vec![
                    Value::from_i32(TypeId::Integer, ((cursor + 2) % 10) as i32),
                    Value::from_i32(TypeId::Integer, cursor as i32),
                    Value::from_i32(TypeId::Integer, ((cursor + 50) % 100) as i32),
                    Value::from_i32(TypeId::Integer, (cursor / 100) as i32),
                    Value::from_i32(TypeId::Integer, 233),
        ...
        "__mock_agg_input_big" => {
            Box::new(move |cursor: usize| {
                let values = vec![
                    ...
                    Value::from_i32(TypeId::Integer, cursor as i32),
                    Value::from_i32(TypeId::Integer, ((cursor + 50) % 100) as i32),
                    Value::from_i32(TypeId::Integer, (cursor / 1000) as i32),
                    Value::from_i32(TypeId::Integer, 233),
        ...
        "__mock_external_merge_sort_input" => {
            Box::new(move |cursor: usize| {
                let values = vec![
                    Value::from_i32(TypeId::Integer, cursor as i32),
                    Value::from_i32(TypeId::Integer, ((cursor + 1777) % 15000) as i32),
                    Value::from_i32(TypeId::Integer, ((cursor + 3) % 111) as i32),
                ];
                Tuple::new_with_values(values, plan.output_schema_ref())
            })
        }
        "__mock_table_123" => {
            Box::new(move |cursor: usize| {
                let values = vec![Value::from_i32(TypeId::Integer, (cursor + 1) as i32)];
                Tuple::new_with_values(values, plan.output_schema_ref())
            })
        }
        "__mock_graph" => {
            Box::new(move |cursor: usize| {
                ...
                let values = vec![
                    Value::from_i32(TypeId::Integer, src as i32),
                    Value::from_i32(TypeId::Integer, dst as i32),
                    ...
                    Value::from_i32(TypeId::Integer, (src * 100) as i32),
                    Value::from_i32(TypeId::Integer, (dst * 100) as i32),
                    if src == dst {
                        ValueFactory::get_null_value_by_type(TypeId::Integer)
                    } else {
                        Value::from_i32(TypeId::Integer, 1)
                    },
                ];
                Tuple::new_with_values(values, plan.output_schema_ref())
            })
        ...
        "__mock_t1" => {
            Box::new(move |cursor: usize| {
                let values = vec![
                    Value::from_i32(TypeId::Integer, (cursor / 10000) as i32),
                    Value::from_i32(TypeId::Integer, (cursor % 10000) as i32),
                    Value::from_i32(TypeId::Integer, cursor as i32),
                ];
                Tuple::new_with_values(values, plan.output_schema_ref())
        ...
        "__mock_t4_1m" => {
            Box::new(move |cursor: usize| {
                let c = cursor % 500000;
                let values = vec![
                    Value::from_i32(TypeId::Integer, c as i32),
                    Value::from_i32(TypeId::Integer, (c * 10) as i32),
                ];
                Tuple::new_with_values(values, plan.output_schema_ref())
            })
        }
        ...
        "__mock_t7" => {
            Box::new(move |cursor: usize| {
                let values = vec![
                    Value::from_i32(TypeId::Integer, (cursor % 20) as i32),
                    Value::from_i32(TypeId::Integer, cursor as i32),
                    Value::from_i32(TypeId::Integer, cursor as i32),
                ];
                Tuple::new_with_values(values, plan.output_schema_ref())
        ...
        "__mock_t8" => {
            Box::new(move |cursor: usize| {
                let values = vec![Value::from_i32(TypeId::Integer, cursor as i32)];
                Tuple::new_with_values(values, plan.output_schema_ref())
            })
        }
        "__mock_t9" => {
            Box::new(move |cursor: usize| {
                ...
                let values = vec![
                    Value::from_i32(TypeId::Integer, (cursor / 10000) as i32),
                    Value::from_i32(TypeId::Integer, y),
                ];
                Tuple::new_with_values(values, plan.output_schema_ref())
            })
        }
        "__mock_t10" => {
            Box::new(move |cursor: usize| {
                let values = vec![
                    Value::from_i32(TypeId::Integer, cursor as i32),
                    Value::from_i32(TypeId::Integer, (cursor * 10) as i32),
                ];
                Tuple::new_with_values(values, plan.output_schema_ref())
            })
        }
        "__mock_t11" => {
            Box::new(move |cursor: usize| {
                let values = vec![
                    Value::from_i32(TypeId::Integer, -1 * (cursor as i32 % 1000) - 1),
                    Value::from_i32(TypeId::Integer, (cursor * 10) as i32),
                ];
                Tuple::new_with_values(values, plan.output_schema_ref())
            })
        ...
        _ => Box::new(move |_cursor: usize| {
            let values: Vec<Value> = plan
                ...
                .get_columns()
                .iter()
                .map(|col| ValueFactory::get_zero_value_by_type(col.get_type()))
                .collect();
            Tuple::new_with_values(values, plan.output_schema_ref())
        }),
    }
...
impl<'a> MockScanExecutor<'a> {
    ...
    pub fn new(plan: &'a MockScanPlanNode) -> Self {
    ...
    /// Initialize the mock scan, resetting the cursor.
    ...
    ///
    /// Returns `true` if a tuple was produced, `false` if there are no more tuples.
    pub fn next(&mut self, tuple: &mut Tuple, rid: &mut RID) -> bool {
        if self.cursor == self.size {
            return EXECUTOR_EXHAUSTED;
        }
        if self.shuffled_idx.is_empty() {
            *tuple = (self.func)(self.cursor);
        } else {
            *tuple = (self.func)(self.shuffled_idx[self.cursor]);
        }
        self.cursor += 1;
        *rid = Self::make_dummy_rid();
        EXECUTOR_ACTIVE
    }

    /// Return the output schema for this scan.
    pub fn output_schema_ref(&self) -> &Schema {
        self.plan.output_schema_ref()
    }
